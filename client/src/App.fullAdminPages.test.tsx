...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ...
    management: { dashboard: { useQuery: () => ({ isLoading: false, data: { metrics: { schoolsWithCycles: 0, submitted: 0, validated: 0, openIssues: 0, totalValue: 0 }, cycles: [], consolidated: [], pendingIssues: [] } }) }, controlExport: { useQuery: () => ({ isFetching: false, refetch: vi.fn() }) }, changeStatus: { useMutation: () => mutation }, listUsers: { useQuery: () => ({ data: [] }) }, setUserRole: { useMutation: () => mutation }, archiveStats: { useQuery: () => ({ isLoading: false, isFetching: false, data: null, refetch: vi.fn() }) }, retryArchive: { useMutation: () => ({ ...mutation, variables: null }) } },
  },
}));
